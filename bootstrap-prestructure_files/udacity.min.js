...
function(b){a.serverError=b.error;a.saveDisabled=!1}):a.saveDisabled=!1}}}]);angular.module("account").controller("developmentEnvironmentInfoTab",["$scope","currentUser","developmentEnvironments","$log",function(a,d){a.accountKey=d.get("key")}]);angular.module("account").controller("linkedAccountsTab",["$scope","$timeout","currentUser","users","google","facebook","normalizedSocialService","session","siteExperience",function(a,d,b,c,e,f,g,h,i){a.loading=!0;a.managed=i.managedAccount;a.accounts={google:null,facebook:null};var j=function(){a.serverSuccess="";a.serverError=""},l=function(b,c,e){c=g.normalizeAccount(b,c);c.id===a.user["_"+b+"_id"]||e?c.loggedIn=!0:c={loggedIn:!1};return a.accounts[b]=c},k=function(b){a.loading=!1;a.serverError=
b.error};a.getLinkedAccountTypes=function(){return _.keys(a.accounts)};a.disconnect=function(b){a.loading=!0;var e={key:a.user.key};e[b+"_id"]=null;c.put(e).then(function(c){j();a.loading=!1;a.user=c;"github"!==b&&(a.accounts[b]=null)},k)};a.disconnectSocial=function(b){a.loading=!0;var e={key:a.user.key},d=a.user.social_accounts.length&&"github"==b?"social_accounts":"external_accounts",f=null;_.each(a.user[d],function(a,c){if(a.account_type===b||a.affiliate_program_key===b)f=c});null==f?a.loading=
!1:(a.user[d].splice(f,1),e[d]=a.user[d],c.put(e).then(function(c){j();a.loading=!1;a.user=c;a[b]=null},k))};a.connect=function(b){a.loading=!0;var e=g.normalizeAccountService(b),f=function(g,h){var i=function(a){401===a.code&&f(g-1,3*h)};0===g&&(i=k);d(function(){e.getAccount().then(function(e){var d={key:a.user.key};d[b+"_id"]=e.id;c.put(d).then(function(c){e=l(b,e,!0);j();a.loading=!1;a.user=c},k)},i)},h)},i=function(){f(3,500)};e.login().then(function(a){a=g.createSessionData(b,a);h.put(a).then(i,
i)},k)};j();b.fetchFullUser().then(function(){a.loading=!1;a.user=b.getUser();_.each(a.user.social_accounts,function(b){a[b.affiliate_program_key]=b});_.each(a.user.external_accounts,function(b){a[b.affiliate_program_key]=b});_.each(a.getLinkedAccountTypes(),function(a){g.normalizeAccountService(a).getAccount().then(function(b){l(a,b)})})},k)}]);angular.module("account").controller("myCertificatesTab",["$q","$scope","$sce","currentUser","users","nodes",function(a,d,b,c,e,f){d.loading=!0;d.verifiedCertificates=[];d.nodeMap={};d.serverError="";var g=c.getUser();d.getBadges=function(){return _.filter(g._badges,function(a){return"Course Mastery"===a.name&&0<a.highest_level})};a.all([e.getCertificates("me").then(function(a){d.verifiedCertificates=a;a=_.map(a,function(a){return a.node_ref.key});return f.batchGet(a,{projection:"catalog"}).then(function(a){_.extend(d.nodeMap,
...
isFreeEnrolled:function(a,c){return!d.isCurrentEnrolled(a,c)&&"enrolled"===a.state},hasEnrolled:function(b,c){return a(b,c,"past_student_group_refs")||d.isGraduated(b,c)},hasPermissionTo:function(a,c){return a&&!!_.find(a.guard.allowed_behaviors,function(a){return a===c})}};return d}]);angular.module("foundation").directive("notificationsDropdown",["notificationsDropdownHelper","layer","$q","currentUser","$timeout","$window","nodes",function(a,d,b,c,e,f,g){return{scope:{},restrict:"A",templateUrl:"/templates/notifications_dropdown/notifications_dropdown.html",link:function(b){var i;d.authenticate().then(function(){i=d.startWebSocketConnection();return c.fetchStudyGroups()}).then(function(c){b.showNotifications=c&&0<c.length;if(b.showNotifications){b.loading=!0;b.nodeMap=null;b.hasSeenAllNotifications=
!1;b.markNotificationsAsRead=function(){b.hasSeenAllNotifications=!0};b.hasUnreadNotification=function(){return _.some(b.nodeMap,function(a){return a.conversation&&0<a.conversation.unread_message_count})};b.goToTeamsPage=function(){f.open("/teams/#chat","_blank")};var d=_.flatten(_.map(c,function(a){return _.map(a._node_refs,function(a){return a.key})})),k=_.indexBy(c,"conversation_id");g.batchGet(d).then(function(d){b.nodeMap=d;a.updateNodeMapWithExistingConversations(b.nodeMap,c).then(function(){e(function(){})});
b.loading=!1;var f=0;i.onmessage=function(c){c=JSON.parse(c.data);a.isUnreadMessageCountPacket(c)&&(a.updateUnreadMessageCountInNodeMap(b.nodeMap,k,c),b.hasSeenAllNotifications&&(c=a.calculateTotalUnreadMessageCountInNodeMap(b.nodeMap),c>f&&(b.hasSeenAllNotifications=!1),f=c),e(function(){}))}})}})}}}]).service("notificationsDropdownHelper",["layer","$q",function(a,d){return{isUnreadMessageCountPacket:function(a){return a.object&&"Conversation"===a.object.type&&"change"===a.type?_.some(a.data,function(a){return"unread_message_count"===
a.property}):!1},updateNodeMapWithExistingConversations:function(b,c){return d.all(_.map(c,function(c){return a.fetchConversation(c.conversation_id).then(function(a){b[c._node_refs[0].key].conversation=a}).catch(function(){})}))},updateUnreadMessageCountInNodeMap:function(a,c,e){if(c=c[e.object.id]){var d=a[c._node_refs[0].key];d&&(d.conversation=d.conversation||{},_.forEach(e.data,function(a){"unread_message_count"===a.property&&(d.conversation.unread_message_count=a.value)}))}return a},calculateTotalUnreadMessageCountInNodeMap:function(a){return _.reduce(a,
function(a,b){return b.conversation&&b.conversation.unread_message_count?a+b.conversation.unread_message_count:a},0)}}}]).service("layer",["$http","$timeout","$q","applicationEnvironment","users",function(a,d,b,c,e){var f={};f.layerAppId=c.isInDevelopment()?"e5fe3bb6-1ef1-11e5-9078-ad56000000fb":"e5fe4034-1ef1-11e5-8bd1-ad56000000fb";f.fetchConversation=function(a){return f.layerGet(f.getUrlFromLayerId(a)).then(function(a){return a.data})};f.getUrlFromLayerId=function(a){return a.split("layer://")[1]};
f.layerGet=function(b,c,e){return a.get("https://udacity-api.layer.com"+b,_.extend({data:c},{headers:f.addLayerHeaders(_.extend(a.defaults.headers.common,e))}))};f.layerPost=function(b,c){return a.post("https://udacity-api.layer.com"+b,c,{headers:f.addLayerHeaders(a.defaults.headers.common)})};f.addLayerHeaders=function(a){return _.extend({},a||{},{Authorization:f.sessionToken?'Layer session-token="'+f.sessionToken+'"':void 0,"X-Udacity-Client":void 0,Accept:"application/vnd.layer+json; version=1.0"})};
f.generateNonce=function(){return f.layerPost("/nonces").then(function(a){return a.data.nonce})};f.authenticate=function(){return f.generateNonce().then(function(a){return f.requestAuthenticationNonceWithCompletion(a)}).then(function(a){f.sessionToken=a})};f.startWebSocketConnection=function(){return new WebSocket("wss://udacity-api.layer.com/websocket?session_token="+f.sessionToken,"com.layer.notifications-1.0")};f.requestAuthenticationNonceWithCompletion=function(a){return e.fetchJWTForLayer(a).then(function(a){return f.layerPost("/sessions",
{identity_token:a,app_id:f.layerAppId})}).then(function(a){return a.data.session_token})};return f}]);angular.module("foundation").controller("siteFrame",["$scope","$window","localStorage",function(a,d,b){a.responsivenessIsEnabled=!1;a.$on("setResponsiveness",function(b,e){!a.responsivenessIsEnabled&&e&&0===$('meta[name="viewport"]').length?$("head").append('<meta name="viewport" content="width=device-width, initial-scale=1.0" />'):$('meta[name="viewport"]').remove();a.responsivenessIsEnabled=e});a.cacheBustingIsEnabled="yes"===b.getItem("bustAllTheCaches")?"yes":"no";a.toggleCacheBusting=function(){a.cacheBustingIsEnabled=
...
"Group"===a.group_ref.ref&&e(a.group_ref.key)});a.ready=!0})}]);angular.module("overview").directive("nanodegreeEnrollmentActions",["cacheVersion","currentUser","groups","$location","dateTime","$window","gaqHelper","cookieJar","googleTagManager","universalAnalytics",function(a,d,b,c,e,f,g,h,i,j){return{restrict:"A",scope:{enrollmentActions:"="},templateUrl:"/templates/overview/nanodegree_enrollment_actions.html?"+a,controller:"enrollmentActionsHelper",link:function(a){var b=function(a){return{nd000:{start:"July 8 at 12:00 pm Eastern Time",end:"July 19 at 11:59 pm Eastern Time"},
nd001:{start:"July 8 at 12:00 pm Eastern Time",end:"July 19 at 11:59 pm Eastern Time"},nd002:{start:"July 8 at 12:00 pm Eastern Time",end:"July 19 at 11:59 pm Eastern Time"},nd003:{start:"July 8 at 12:00 pm Eastern Time",end:"July 19 at 11:59 pm Eastern Time"},nd004:{start:"July 8 at 12:00 pm Eastern Time",end:"July 19 at 11:59 pm Eastern Time"},nd801:{start:"July 8 at 12:00 pm Eastern Time",end:"July 19 at 11:59 pm Eastern Time"}}[a]||{}},c=function(){i.pushTrackingEvent("nanodegreeAccess");f.location.href=
"/me"};a.getNanodegreeButtonInfo=function(){return a.isAccessible&&(a.isPaidEnrolled||a.isPaidGraduated)?{variant:"nanodegree is live, student is enrolled or graduated",text:"Continue Nanodegree",action:c}:a.isPaidEnrollable?a.isClassFull?a.isPaidWaitlisted?(h.exists("udacity_notify_me_clicked_recently")&&(i.pushTrackingEvent("notificationListJoin"),g.pushTrackingEvent("Waitlist","SignUp",a.courseKey),j.trackEvent({category:"Waitlist",action:"SignUp",label:a.courseKey}),h.remove("udacity_notify_me_clicked_recently")),
{variant:"nanodegree is full, student is waitlisted",statusMessage:"You will be emailed when enrollment opens "+b(a.courseKey).start,statusSuccess:!0}):{variant:"nanodegree is full, student not waitlisted, not enrolled",text:"Notify me",sidenote:"when nanodegree re-opens",statusMessage:"<strong>Next enrollment opens "+b(a.courseKey).start+".</strong><br>You can start the program immediately after you enroll.",action:function(){h.set("udacity_notify_me_clicked_recently","yes",{expires:new Date((new Date).getTime()+
6E5)});a._joinGroup(a.waitlistGroupKey)}}:{variant:"nanodegree live, student not enrolled",text:"Enroll in Nanodegree",sidenote:"After 1 week free trial",statusMessage:"<strong>Enrollment closes "+b(a.courseKey).end+".</strong><br>You can start the program immediately after you enroll. ",action:function(){i.pushTrackingEvent("nanodegreeEnrollment");"nd801"==a.courseKey?f.location.href="/nanodegree/android-expectation-quiz":a._purchaseCourse()}}:{variant:"nanodegree not open for paid enrollment",sidenote:"This nanodegree is not yet available for enrollment"}};
...
(a.fieldErrors=e.errorText,a.buttonDisabled=!1)};a.signInFacebook=function(){return j.getAuthResponse().then(function(a){return c.put({facebook:{signed_request:a.signedRequest}}).then(p,r)},n)};a.signInGoogle=function(){i.getToken(!1).then(function(a){return c.put({google:{token:a.code}}).then(p,r)},n)}}]);angular.module("account").controller("signUpForm",["$scope","$q","$sce","$timeout","session","users","currentUser","groups","validator","google","facebook","$location","safelyRedirect","gaqHelper","googleTagManager","retry","cookieJar","universalAnalytics",function(a,d,b,c,e,f,g,h,i,j,l,k,m,p,n,r,q,t){a._removeEmailCookie=function(){q.exists("emailAddressForSignUp")&&q.remove("emailAddressForSignUp",{path:"/"})};var u=function(){var b=function(){var b=a.initialSearch.next;b?m(b):m("/")},c=a.initialSearch.enroll,
e=a.initialSearch.enroll_state||"enrolled",f=a.initialSearch["join-group"],e=c?g.enroll(c,e,!0):d.when(!0),i=f?g.fetchFullUser().then(function(){return h.addMember(f,g.get("key"))}):d.when(!0),j,k;a._removeEmailCookie();c?(j=p.pushTrackingEvent("SignUp","Enroll",c),t.trackEvent({category:"SignUp",action:"Enroll",label:c})):(j=p.pushTrackingEvent("SignUp","Direct"),t.trackEvent({category:"SignUp",action:"Direct"}));k=n.pushTrackingEvent("RegistrationCompleted");return e["finally"](function(){return i["finally"](function(){return j["finally"](function(){return k["finally"](b)})})})},
...
a.save=function(){a.serverSuccess=a.serverError="";a.saving=!0;e.site_preferences={use_html5_player:a.settings.useHtml5Player,auto_advance:a.settings.autoAdvance,playback_rate:a.settings.playbackRate.value,video_quality:a.settings.videoQuality.value};b.put(e).then(function(){a.saving=!1;a.serverSuccess="Classroom Settings Saved"},function(b){a.saving=!1;a.serverError=b.error})}}]);angular.module("payment").controller("checkoutPage",["$scope","paymentFlow","$routeParams","$location","api","gaqHelper","currentUser","cacheVersion","modal","dateTime","optimizely","$timeout","universalAnalytics",function(a,d,b,c,e,f,g,h,i,j,l,k,m){g.get("_registered")||(window.location.href="/account/auth#!/signup?next="+encodeURIComponent(window.location.href));a.$on("$routeChangeSuccess",function(){f.pushPageviewEvent();m.trackVirtualPageView();if(angular.isDefined(b.stepNumber)&&d.isInFlow())d.goToStep(b.stepNumber);
else{var e=[];c.search().products?(e=c.search().products.split(","),d.begin(e),a.flow=d.getCurrentFlowData()):window.history.back()}});a.getCurrentDate=function(){return j.getCurrent()};a.isProductLaunched=function(a){return a&&a.launch_date&&new Date(a.launch_date)<j.getCurrent()};a.isProductNotLaunched=function(a){return a&&!a.was_launched};a.getGoToNanodegreeLink=function(){return a.flow&&"nd001"===a.flow.uriEncodedCourseKey?"/teams/#/onboarding/"+a.flow.uriEncodedCourseKey:"/me"};a.getDatePlusDays=
function(a,b){return a?new Date(a.getTime()+864E5*b):null};a.step={};a.submitStep=function(b){a.step.submitting=!0;d.submitStep(b).then(function(){a.step.submitting=!1;$("html,body").animate({scrollTop:0})})};a.getCacheBustingUrl=function(a){return a&&a+"?"+h};a.currentUserName=g.getFullName();a.currentUserEmail=g.get("email").address;a.currentUserNickname=g.get("nickname");a.getCurrentUserIsVerified=function(){return g.get("email")._verified};a.getCurrentStep=d.getCurrentStep;a.getCurrentFlowSteps=
d.getCurrentFlowSteps;k(function(){!a.getCurrentUserIsVerified()&&!window.optimizelyDisableVerificationModal&&i.activate("verify email")},1);a.$on("$includeContentLoaded",function(){l.activate()})}]);angular.module("page.payment",["core"]).config(["$routeProvider",function(a){a.when("/:stepNumber",{}).otherwise({redirectTo:"/0"})}]);angular.module("payment").constant("DAY_IN_MILLISECONDS",864E5).factory("fullClassExperienceFlowDefinition",["$q","stripeCreditCard","validator","api","currentUser","products","customers","couponCode","users","$timeout","$log","nodes","DAY_IN_MILLISECONDS","optimizely","userTrials","googleTagManager","universalAnalytics",function(a,d,b,c,e,f,g,h,i,j,l,k,m,p,n,r,q){var t=function(a){return"card_declined"===a.code?"Your card was declined. Please try again. If this issue persists, please contact payment-support@udacity.com.":
...
{valid:!1};return null});i.push(j)}a.all(i).then(function(a){e.product=a[0];var b=_.first(e.product._course_groups).course_ref.key;k.get(b).then(function(a){var b=a.catalog_entry._image?a.catalog_entry._image.serving_url:null;e.confirmationStepLabel={Degree:"Start Nanodegree",Course:"Start course"}[a._semantic_type];e.course={key:a.key,imageUrl:b,isNanodegree:"Degree"===a._semantic_type};r.pushTrackingEvent("checkoutStart",{productName:e.product.name,productPrice:e.product.amount,orderNodeKey:e.course.key,
isNanodegree:"Degree"===a._semantic_type})});b=e.product.trial_period_days;e.inTrialPeriod=0<b;e.trialPeriodStarts=new Date(Date.now()+b*m);e.customer=a[1];e.customerCard=d.getDefaultCard(e.customer);e.cardIsLoaded=!0;e.uriEncodedCourseKey=window.encodeURIComponent(u(e.product));p.activate()})},onJump:function(a){return a.paymentConfirmed?!1:!a.paymentConfirmed},onSubmit:function(b,c){if(c.customerCard)return v(!1,c);var e=b.creditCard||{},f=d.checkValidity(e);c.creditCardErrors=f.errorText;if(!f.valid)return!1;
f=(c.product&&c.product.name)+", "+u(c.product);return d.createPaymentToken(f,e).then(function(a){return v(a,c)},function(b){c.generalPaymentFormError=t(b.error);return a.reject(b)})}},{getLabel:function(a){return a.confirmationStepLabel},view:"/templates/angular/payment/payment_confirmation.html",onStart:function(a,b){b.paymentConfirmed=!0;b.payment=a;var c=b.product&&(b.product.amount||0)/100,d=null;try{d=_.find(a.customer.subscriptions,function(a){return a.plan.id===b.product.key}).id}catch(e){l.error("Failed to get Transaction Id",
e)}window._gaq.push(["_addTrans",d,"Udacity",c,"0.00","0.00","Mountain View","California","USA"]);window._gaq.push(["_addItem",d,"DD44",b.product&&b.product.name,u(b.product),c,"1"]);window._gaq.push(["_trackTrans"]);q.trackTransaction({transactionId:d,transactionTotal:c,transactionAffiliation:"Udacity",transactionProducts:[{name:b.product&&b.product.name||"Missing product name, FILL IN IN CONTENT EDITOR ASAP FOR "+_.first(u(b.product)),sku:"DD44",price:c,quantity:1,category:_.first(u(b.product))}]});
...
g.pending[e]&&g.pending[e].readyNotification(g.run);if(null!==b)g.pending=_.without(g.pending,b),b["finally"](g.run),g.running.push(b),d(b.start);else break}}};return g};c=a.make;return a}]);angular.module("user").factory("currentUser",["$log","$q","$rootScope","users","basicUser","gaqHelper","nodes","nodeUtility","userUtility","cohortsGateway","universalAnalytics",function(a,d,b,c,e,f,g,h,i,j,l){var k=angular.copy(e),m=null,p=null,n={get:function(a){return k[a]},getUser:function(){return k},getFullName:function(){return i.getFullName(k)},isStaff:function(){if(k._memberships)return 0<_.filter(k._memberships,function(a){return-1<a.group_ref.key.indexOf("staff")}).length},getEnrolledCourses:function(){return _.chain(n.get("_enrollments")).filter(function(a){if(a&&
"enrolled"===a.state)return!0}).map(function(a){return a.node_key}).value()},getEnrolledNanodegreeKeys:function(){return _.filter(n.getEnrolledCourses(),function(a){return 0===a.indexOf("nd")})},fetchGraduatedNanodegreeKeys:function(){return n.fetchCertificates().then(function(a){return _.chain(a).map(function(a){return a.node_ref.key}).filter(function(a){return 0===a.indexOf("nd")}).value()})},fetchGraduatedCourseKeys:function(){return n.fetchCertificates().then(function(a){return _.map(a,function(a){return a.node_ref.key})})},
fetchEnrolledAndGraduatedCourseKeys:function(){return n.fetchFullUser().then(function(){return n.fetchGraduatedCourseKeys()}).then(function(a){return _.union(n.getEnrolledCourses(),a)})},fetchEnrolledAndGraduatedNanodegreeKeys:function(){return n.fetchGraduatedNanodegreeKeys().then(function(a){return _.union(n.getEnrolledNanodegreeKeys(),a)})},getGroupKeys:function(){return _.chain(n.get("_memberships")).filter(function(a){return a.group_ref&&"Group"===a.group_ref.ref}).map(function(a){return a.group_ref.key}).value()},
fetchStudyGroups:function(){return c.getStudyGroups("me")},getEmail:function(){return i.getEmail(k)},setUser:function(a){angular.copy(angular.copy(a),k)},fetchFullUser:function(a){var b=a?{cacheBust:Math.random()}:{};if(a)return c.get("me",b).then(function(a){n.setUser(a);return a});m||(m=c.get("me",b).then(function(a){n.setUser(a);return a}));return m},fetchCertificates:function(){return k._certificates?d.when(k._certificates):c.getCertificates("me").then(function(a){return k._certificates=a})},
checkIfFirstChatVisit:function(){return c.checkIfFirstChatVisit("me")},setFirstChatVisit:function(){return c.setFirstChatVisit("me")},checkNewHomeBanner:function(){return c.checkNewHomeBanner("me")},setNewHomeBanner:function(){return c.setNewHomeBanner("me")},fetchCohorts:function(){p||(p=n.fetchFullUser().then(function(a){return d.all(_.map(a._cohort_keys,function(a){return j.get(a)}))},function(b){a.error(b)}));return p},userIsLoggedIn:function(){return k._registered},isEnrolledIn:function(a){return _.some(k._enrollments,
...
i=null,j=0,l=f.length;j<l;j++){i=f[j].match(/([^=]+)=/i);if(angular.isArray(i))try{g=decodeURIComponent(i[1]),h=!1===c?f[j].substring(i[1].length+1):decodeURIComponent(f[j].substring(i[1].length+1))}catch(k){}else g=decodeURIComponent(f[j]),h="";d[g]=h}return d};return{exists:function(a){if(!angular.isString(a)||""===a)throw new TypeError("$cookieJar.exists(): Cookie name must be a non-empty string.");return d(document.cookie,!0).hasOwnProperty(a)},get:function(a,c){var c=angular.isObject(c)?c:{},
...
b.$on("$destroy",function(){a.detach()})}}}]);angular.module("viewer").directive("contentGateWatcher",["lesson","currentUser","imageQuiz","programmingQuiz","$location","modal",function(a,d,b,c,e,f){return{restrict:"A",link:function(e){var h=d.userIsLoggedIn(),i=!1,j=function(){b.removeEventListener("correct",j);c.removeEventListener("evaluated",l);h?f.activate("enrollGate"):f.activate("signInGate")},l=function(a){a.result&&a.result.passed&&j()},k=function(){b.addEventListener("correct",j);c.addEventListener("evaluated",l)};a.addEventListener("courseChange",
...
!0,a=$.extend({},a),c.init(a))},{replayLast:!0,removeOnDestructionOf:b})},{removeOnDestructionOf:b,removeAfterFired:!0});d.addEventListener("tidbitChange",function(a){b.tidbitViewerMode=e.getTidbitViewerMode(a.tidbit)},{replayLast:!0,removeOnDestructionOf:b});var h=g.find(".code-mirror-holder"),i=g.find(".editor-file-tabs"),j=function(){h.outerHeight(g.height()-(b.files&&1<b.files.length?i.outerHeight():0))},l=_.debounce(j,750),k=_.throttle(j,200);$(window).on("resize",l);$(window).on("resize",k);
...
b.showFeedbackDialog=function(){b.morselTitle=b.morsel.title;"Video"===b.morsel.model?(g.pauseVideo(),b.feedbackOptions=j):b.feedbackOptions="ImageFormQuiz"===b.morsel.model||"ProgrammingQuiz"===b.morsel.model?l:k;c.activate("feedback")};var m=function(a){for(var a=_.pairs(a),b={},c=0;c<a.length;++c){var d=a[c][0],e=a[c][1];if(_.isObject(e))for(var e=_.pairs(m(e)),f=0;f<e.length;++f)b[d+"."+e[f][0]]=e[f][1];else void 0!==e&&null!==e&&(b[d]=e)}return b};b.submitFeedback=function(){b.formValidationResults=
...
a.get("/nodes/"+b+"/state?fresh=true")]).then(function(a){if(e===m){var d=a[0].data,f=a[1].data,a=null;j=d.references.Node[b];l=d.references.Node;j.steps_refs=_.reject(j.steps_refs,function(a){return!l[a.key]});_.each(j.steps_refs,function(a,b){var c=l[a.key];if(!c)throw Error("Expected a morsel but got "+JSON.stringify(c,2));"Exercise"===c.model&&(_.each(["lecture_ref","quiz_ref","answer_ref"],function(a){c[a]&&(c[a]=l[c[a].key],c[a]&&(c[a].containingExercise=c))}),_.each(c.tidbits,function(a){a.tidbit_ref=
l[a.tidbit_ref.key];a.tidbit_ref&&(a.tidbit_ref.containingExercise=c)}));l[a.key].indexInLesson=b});_.each(l,function(a){if("ImageFormQuiz"===a.model||"ProgrammingQuiz"===a.model)a.binCodes=[]});if(d=_.find(f.nodestates,function(a){return a.node_ref.key===b}))_.each(d.completed_descendant_node_refs,function(a){n.markMorselAsCompleted(a.key)}),(d=g(d.last_interactions,c.getCurrent()))&&d.morselKey&&(a=l[d.morselKey]);n.emitEvent("lessonChange",{lesson:j,morsels:_.map(j.steps_refs,function(a){return l[a.key]}),
lastMorselVisited:a})}},function(a){e===m&&a&&(a.status&&(403===a.status||404===a.status))&&n.emitEvent("lessonPermissionDenied",b)})},announcePendingMorselChange:function(a){n.emitEvent("pendingMorselChange",{from:k,to:a})},changeMorsel:function(a){k=l[a];n.emitEvent("morselChange",k)},changeTidbit:function(a){n.emitEvent("tidbitChange",{tidbit:a?l[a]:null})},markMorselAsCompleted:function(a){l[a]?l[a].completed=!0:console.error("Can't find morsel #"+a+" to mark it as completed")},updateProgressForMorsel:function(a,
...
function(a){j=a},{replayLast:!0});c.addEventListener("videoPause",function(a){null!==j&&(null!==a&&void 0!==a)&&(l=a,k([{context:b.getCurrent(),data:{model:"PlayableView",start:j,stop:l}}]));j=l=null},{replayLast:!0});f.addEventListener("evaluation",i,{replayLast:!0});g.addEventListener("evaluation",i,{replayLast:!0});d.addEventListener("morselChange",function(a){if(!a.completed&&("Reading"===a.model||"EmbeddedFrame"===a.model))d.markMorselAsCompleted(a.key),k([{context:b.getCurrent(),data:{model:"CompletedExperience",
...
{projection:"image"}).then(function(b){b&&b._image_url&&a.partners[d].push({imageUrl:b._image_url,name:b.name})})})};a.fetchCards=function(b,c){return d.batchGet(b).then(function(d){return _.without(_.map(b,function(b){var e=!1,j=d[b];if(d[b]){var l=null,k=j.catalog_entry;k&&k._image&&(l=k._image.serving_url);c&&(j.affiliate_program_refs&&0<j.affiliate_program_refs.length)&&"udacity"!==j.affiliate_program_refs[0].key&&(e=!0,a._fetchAffiliate(j.affiliate_program_refs,b));return{key:b,_slug:j._slug,
servingUrl:l,title:j.title,isNanodegree:"Degree"===j._semantic_type,showPartner:e&&c}}return null}),null)})};a._setLengthOfRow=function(a,b){return!b||6<b?6>=a?a:0===a%6?6:0===a%5?5:0===a%4?4:0===a%3?3:6:b};a.setUpCards=function(b,d,g,h,i){b?(a.lengthOfRow=a._setLengthOfRow(b.length,d),a.columnWidthClass="col-xs-"+a._calculateColumnWidth(a.lengthOfRow),a.cardImageSize=[1200/a.lengthOfRow,0],angular.isArray(h)&&2===h.length?a.cardImageSize=h:angular.isNumber(h)&&(a.cardImageSize=[h,0]),i?1===i?a.columnWidthClass=
